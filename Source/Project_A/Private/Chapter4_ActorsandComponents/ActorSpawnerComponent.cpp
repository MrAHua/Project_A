...
#include "Chapter4_ActorsandComponents/ActorSpawnerComponent.h"
...
void UActorSpawnerComponent::Spawn()
{
	...
	if (TheWorld != nullptr)
	{
		...
		AActor* MyActor = TheWorld->SpawnActor(ActorToSpawn, &ComponentTransform);
		//MyActor->AttachToActor(GetOwner(), FAttachmentTransformRules::KeepRelativeTransform);
		MyActor->AttachToActor(GetAttachmentRootActor(), FAttachmentTransformRules::KeepWorldTransform);
	}
}
